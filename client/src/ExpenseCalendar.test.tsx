...
describe('ExpenseCalendar', () => {
  it('renders one cell per day of the month, with the 1st under the correct weekday', () => {
    ...
    const { container } = render(
      <ExpenseCalendar month="2026-01" expenses={[]} onDayClick={vi.fn()} />
    )
  ...
  it("shows a day's total for a day with expenses, and just the day number for a day without", () => {
    ...
    render(<ExpenseCalendar month="2026-01" expenses={expenses} onDayClick={vi.fn()} />)
  ...
  it('calls onDayClick with the same day on every click, including repeated clicks', () => {
    const onDayClick = vi.fn()
    const expenses = [expense({ date: '2026-01-05T00:00:00.000Z', amount: 10 })]

    render(<ExpenseCalendar month="2026-01" expenses={expenses} onDayClick={onDayClick} />)

    fireEvent.click(screen.getByRole('button', { name: /5/ }))
    expect(onDayClick).toHaveBeenCalledWith('2026-01-05')
